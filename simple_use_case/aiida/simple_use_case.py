...
@shellfunction(command="gmsh", output_filenames=["mesh.msh"])
def gmsh(**kwargs):
    """Run the ``gmsh`` command."""


@shellfunction(command="meshio", output_filenames=["mesh.xdmf"])
def meshio(**kwargs):
...
def workflow(geometry, poisson_script, post_processing_script):
    ...
    arguments = List(["-2", "-order", "1", "-format", "msh2", "{geometry}"])
